<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        @if(auth()->user()->role === 'admin')
                            ...
                            {{-- 'Tambah User' removed from desktop nav per request --}}
                            <x-nav-link :href="route('users.import')" :active="request()->routeIs('users.import')">
                                {{ __('Import User') }}
                            </x-nav-link>
                        @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                    @if(auth()->user()->role === 'admin')
                    <x-responsive-nav-link :href="route('users.index')" :active="request()->routeIs('users.index') || request()->routeIs('users.show') || request()->routeIs('users.edit')">
                        {{ __('Manajemen User') }}
                    </x-responsive-nav-link>
                    {{-- Mobile "Tambah User" link hidden per request --}}
                    {{-- <x-responsive-nav-link :href="route('users.create')" :active="request()->routeIs('users.create')">
                        {{ __('Tambah User') }}
                    </x-responsive-nav-link> --}}
                    <x-responsive-nav-link :href="route('users.import')" :active="request()->routeIs('users.import')">
                        {{ __('Import User') }}
                    </x-responsive-nav-link>
                @endif
                @if(auth()->user()->role === 'dosen')
